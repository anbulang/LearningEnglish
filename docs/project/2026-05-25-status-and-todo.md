...
当前处于 **MVP 主链已打通，真实媒体 provider readiness 待补截图，HN-017 stub 口语评分闭环已落地但真实 provider 与物理手机录音截图待补** 的阶段。

一句话概括：

> 登录、上传、AI 校对、课程详情、复习、stub 口语评分、周报这条链已经具备代码、测试和 Harness 抓手；当前最需要收口的是 HN-016 / HN-016A 真实媒体证据，以及 HN-017 物理手机录音提交截图和真实语音评分 provider 方案。
...
| `HN-017` 孩子录音上传与 AI 语音评分 | stub 闭环已完成，真实 provider 未完成 | 已接入 multipart 录音上传、音频 storage、worker stub 评分、结果页轮询和周报回填；API/worker/storage 与真机安装连通证据已补，物理手机录音截图待补 |
...
- speaking 已具备录音上传、音频 storage、stub 异步评分、结果页和周报回填；2026-05-25 已补 API/worker/storage 证据和真机安装启动/局域网连通证据，仍缺真实语音评分 provider 实现、接入验收和物理手机录音提交截图。
...
- speaking 入口已推进到“可录、可传、stub 可评、可展示”；下一步要补真机证据，并把真实语音评分 provider 从配置骨架推进到可运行实现。
...
- [ ] 实现真实 speech assessment provider，至少补齐签名/请求适配、错误处理和失败可见约束，不能只停留在 `AliyunSpeechAssessmentProvider` 骨架。
- [ ] 补齐 HN-017 物理手机录音提交和结果页截图证据；API multipart 上传、worker 日志、attempt JSON、真机安装启动和局域网连通证据已补。
- [ ] 定义 HN-018 周报字段：学习资产掌握度、复习完成度、口语表现、家长建议。

完成标准：

- 学习资产不再停留在“可展示、可听”，而是进入“可练、stub 可评、可汇总”，随后再补真实语音评测。
...
2. 然后实现并验收真实 speech assessment provider，再补齐 HN-017 物理手机录音提交和结果页截图，确认 speaking 结果页在真实设备上可用。
...
- 当前最该优先做的不是继续堆新页面，而是把 `HN-016` / `HN-016A` 从代码能力补到 readiness 证据，并把 `HN-017` 从 stub 闭环推进到真实 provider + 物理手机录音截图证据。
