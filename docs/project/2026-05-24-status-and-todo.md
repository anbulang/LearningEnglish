# LearningEnglish 项目进度与 ToDo（2026-05-24）
...
当前处于 **MVP 主链已打通，文档真相源已完成第二轮对齐，正在补真实媒体 provider readiness 和下一阶段学习闭环** 的阶段。

一句话概括：

> 登录、上传、AI 校对、课程详情、复习、口语入口、周报这条链已经具备代码、测试和 Harness 抓手；当前最需要收口的是 HN-016 / HN-016A 真实媒体证据、HN-017 语音评分闭环、HN-018 报告深化，以及 Android 与证据归档稳定性。
...
| `HN-017` 孩子录音上传与 AI 语音评分 | 未开始 | speaking 入口已有，真实录音与评分未完成 |
...
- speaking 入口已存在，但缺少真实录音数据与结果页面，因此“可练”还没有进入“可评”阶段。
...
- [ ] 设计并实现孩子录音上传接口、对象存储落盘和移动端上传入口。
- [ ] 设计 speaking 结果合同，至少包括转写文本、评分、维度反馈和失败态。
...
2. 然后推进 `HN-017` 录音上传与 AI 评分，让 speaking 从入口变成闭环。
...
- 当前最该优先做的不是继续堆新页面，而是把 `HN-016` / `HN-016A` 从代码能力补到 readiness 证据，再推进 `HN-017` / `HN-018`。
