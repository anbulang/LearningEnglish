...
def enqueue_phonics_attempt_job(attempt_id: str) -> None:
    if get_settings().app_env == "testing":
        logger.info("test environment skipped phonics attempt enqueue %s", attempt_id)
        return
    try:
        from celery import Celery
    except ImportError as exc:
        raise RuntimeError("Celery is required to enqueue phonics attempt jobs") from exc
    ...
    celery_app.send_task("phonics.score_attempt", args=[attempt_id], queue="learning_english")
    logger.info("enqueued phonics attempt job %s", attempt_id)
