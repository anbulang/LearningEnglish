...
def enqueue_phonics_unit_media_job(unit_id: str) -> None:
    if os.getenv("APP_ENV") == "testing":
        logger.info("test environment skipped phonics unit media enqueue %s", unit_id)
        return

    try:
        from celery import Celery
    except ModuleNotFoundError as exc:
        raise RuntimeError("Celery is required to enqueue phonics unit media jobs") from exc

    broker_url = os.getenv("CELERY_BROKER_URL") or os.getenv("REDIS_URL") or "redis://localhost:6379/0"
    result_backend = os.getenv("CELERY_RESULT_BACKEND") or _default_result_backend(broker_url)
    celery_app = Celery("learning_english_api", broker=broker_url, backend=result_backend)
    celery_app.conf.update(task_default_queue="learning_english")
    celery_app.send_task("phonics.process_unit_media", args=[unit_id], queue="learning_english")
    logger.info("enqueued phonics unit media job %s", unit_id)
